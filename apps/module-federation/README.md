...
Federation `name`. Federation support is configured under Uniwind's
`experimental.federation` option:
...
withUniwindConfig(config, {
    ...
    experimental: {
        federation: isRemote
            ? { role: 'remote', id: federation.name, sharedClassNames }
            : { role: 'host', sharedClassNames },
    },
})
```

The host keeps the normal `__reinit` path. Remote CSS modules emit
