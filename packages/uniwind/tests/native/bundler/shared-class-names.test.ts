...
describe('federated shared class names', () => {
    ...
    test('validates complete shared candidates', () => {
        ...
        ).toThrow('experimental.federation.sharedClassNames')
    })

    test('rejects the legacy root-level federation config', () => {
        expect(() =>
            UniwindBundlerConfig.fromMetroConfig({
                cssEntryFile: './unused.css',
                federation: {
        ...
        ).toThrow('federation has moved to experimental.federation')
    })
